...
- (void)viewDidLoad {
    ...
    [self initSessionConfiguration];          // 配置session
...
- (void)addObserver {
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(StatusBarDidRotation:) name:UIApplicationDidChangeStatusBarOrientationNotification object:nil];                      // 监听状态栏位置发生变化的通知
}
...
#pragma mark configuration session
...
//    if (_session.rtmpSessionState == PLVSessionStateNone) {     // 开启推流等待状态时不允许转屏
//        return NO;
//    }else {
//        return YES;
//    }
...
// 设备旋转，可能判断不准确
//- (void)deviceDidRotation:(NSNotification *)nf {
//}

- (void)StatusBarDidRotation:(NSNotification *)nf {
...
- (IBAction)streamButton:(id)sender {
    ...
    switch( _session.rtmpSessionState ) {
        ...
        default: {
            ...
            NSLog(@"%@:%@",NSStringFromCGSize(_videoSize),NSStringFromCGSize([self autoSuitableSize:_videoSize]));
